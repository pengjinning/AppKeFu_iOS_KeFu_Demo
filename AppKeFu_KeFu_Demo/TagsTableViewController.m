...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0)
    {
        ...
        cell.detailTextLabel.text = [APPKEFU getTagNickname];
    }
    else if (indexPath.row == 1)
    {
        cell.textLabel.text = @"2.性别";
        cell.detailTextLabel.text = [APPKEFU getTagSex];
    }
    else if (indexPath.row == 2)
    {
        cell.textLabel.text = @"3.语言";
        cell.detailTextLabel.text = [APPKEFU getTagLanguage];
    }
    else if (indexPath.row == 3)
    {
        cell.textLabel.text = @"4.城市";
        cell.detailTextLabel.text = [APPKEFU getTagCity];
    }
    else if (indexPath.row == 4)
    {
        cell.textLabel.text = @"5.省份";
        cell.detailTextLabel.text = [APPKEFU getTagProvince];
    }
    else if (indexPath.row == 5)
    {
        cell.textLabel.text = @"6.国家";
        cell.detailTextLabel.text = [APPKEFU getTagCountry];
    }
    else if (indexPath.row == 6)
    {
        cell.textLabel.text = @"7.其他";
        cell.detailTextLabel.text = [APPKEFU getTagOther];
    }
